...
module id_sub(
    ...
    assign ri_exception = instvalid == `InstInvalid ? 1'b1 : 1'b0;
    
    assign exception_type = {17'b0, eret_exception, 3'b0, ri_exception, break_exception, syscall_exception, 3'b0, adel_exception, 4'b0};
		  ...
		  `EXE_SPECIAL_INST: begin
		      if (shamt == 5'b00000) begin
		          case (funct)
                      ...
                      // 条件移动由于ID的两个子部件存在相关问题，所以不能在此处立刻判断，因此先默认条件满足，再在执行阶段进行进一步判断
                      `EXE_MOVN: begin
                          we_o = `WriteEnable;
                          aluop_o = `EXE_MOVN_OP;
                          alusel_o = `EXE_RES_MOVE;
