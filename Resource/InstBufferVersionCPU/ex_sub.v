...
module ex_sub(
    ...
    always @ (*) mem_re_o = mem_re & ~|exception_type_o;
    always @ (*) mem_we_o = mem_we & ~|exception_type_o;
        ...
        if (rst == `RstEnable) begin
            ...
            case (aluop_i)
                `EXE_J_OP: begin
                    ...
                    pred_flag = `InvalidPrediction; // 静态分支预测，J指令必预测失败，下面同理。保留该字段是为了后面接入分支预测器方便
                end
                `EXE_JAL_OP: begin
                    branch_flag_actual = `Branch;
                    npc_actual = {pc_4[31:28], imm_i[27:0]};
                    pred_flag = `InvalidPrediction;
                end
                `EXE_JR_OP: begin
                    branch_flag_actual = `Branch;
                    npc_actual = reg1_i;
                ...
                `EXE_BEQ_OP: begin
                    ...
                    pred_flag = branch_flag_actual == `Branch ? `InvalidPrediction : `ValidPrediction;
                end
                `EXE_BGTZ_OP: begin
    ...
    always @ (*) begin
        if (rst == `RstEnable) begin
            ...
            `EXE_LH_OP: begin
                ...
                mem_we = `WriteDisable;
                mem_re = `ReadEnable;
                adel_exception = mem_addr_o[0] != 1'b0;
            end
            `EXE_LHU_OP: begin
                mem_raddr_o = mem_addr_o;
                mem_we = `WriteDisable;
